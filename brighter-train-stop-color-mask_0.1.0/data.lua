...
local function replace_filenames_in_layer(layer)
	if layer.filenames then
		for i, filename in pairs(layer.filenames) do
			layer.filenames[i] = string.gsub(filename, "__base__", "__brighter-train-stop-color-mask__")
		end
	end
	if layer.filename then
		layer.filename = string.gsub(layer.filename, "__base__", "__brighter-train-stop-color-mask__")
	end
end
...
-- Replace the color mask image file names of the train-stop prototype.
local train_stop = data.raw["train-stop"]["train-stop"]
replace_mask_image_filenames(train_stop.top_animations.north.layers)
replace_mask_image_filenames(train_stop.top_animations.east.layers)
replace_mask_image_filenames(train_stop.top_animations.south.layers)
replace_mask_image_filenames(train_stop.top_animations.west.layers)