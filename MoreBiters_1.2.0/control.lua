...
function maybeTunnel(entity)
  if ((game.tick + shiftTick) % 100 == 0 and noSeismic(entity.position)) then -- chance of tunneling on damage
...
function maybeSpawnMimic(tree)
  mimicCounter = mimicCounter + 1
  if (mimicCounter % 80 ~= 0) then
    return
  end
...
function getTunnelPos(entity)
  local radius = 10 + (game.tick % 15) -- 10 to 25
  local tunnelPos = {
    x = entity.position.x,
    y = entity.position.y
  }
